...
MIN_IP_QUALITY_SCORE = 55      # 普通节点最低 IP 质量分
MAX_NODES_PER_EXIT_IP = 2     # 同一个出口 IP 最多保留几个不同端口/节点
...
def filter_ip_quality(unique_nodes: list, residential: list):
    ...
    for n in unique_nodes:
        ...
        if n in residential:
            filtered.append(n)
            continue
        if n["ip_quality_score"] >= MIN_IP_QUALITY_SCORE:
            filtered.append(n)
    ...
    for ip, items in by_ip.items():
        # 家宽前面已经每 IP 只保留一个；这里对普通节点最多保留 N 个。
        items.sort(key=lambda x: (-x.get("ip_quality_score", 0), x.get("latency_ms", 99999)))
        kept = items[:MAX_NODES_PER_EXIT_IP]
        limited.extend(kept)
        ip_dup_dropped += max(0, len(items) - len(kept))

    print(f"[*] 出口 IP 限制: {len(filtered)} → {len(limited)} (同 IP 最多 {MAX_NODES_PER_EXIT_IP} 个，剔除 {ip_dup_dropped})")
...
def classify_and_export(test_results: list):
    ...
    print(f"[*] MITM 劫持高风险节点已剔除: {mitm_dropped} | 稳定性不足剔除: {stability_dropped}")
