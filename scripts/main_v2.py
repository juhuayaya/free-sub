...
def build_run_report(previous_active, current_nodes, test_results, candidates_count, raw_count, parsed_count, residential_count):
...
def print_run_report(report):
    ...
    print(f"最终节点: {report['final_nodes']} | 家宽/移动: {report['residential_nodes']}")
    print("协议成功率:")
...
def classify_and_export(test_results: list):
    ...
    print(f"[*] IP 质量过滤后: 家宽/移动 {len(residential)} | 普通 {len(non_residential)} | 总计 {len(unique_nodes)}")
...
def export_all(unique_nodes, residential, non_residential):
    ...
    # 3) 非家宽总订阅
    # 非家宽 = 最终 unique_nodes 中不属于 residential/mobile 的节点。
    # 这里已经提前排除了广播/Anycast/CDN，因此只保留可用的原生 IDC/其他非家宽出口。
    nonres_links, nonres_proxies, nonres_sb = build_group(non_residential)
    with open(os.path.join(OUTPUT_DIR, "non-residential.txt"), "w", encoding="utf-8") as f:
        f.write(base64.b64encode("\n".join(nonres_links).encode()).decode())
    export_clash_yaml(nonres_proxies, os.path.join(OUTPUT_DIR, "non-residential-clash.yaml"))
    export_singbox_json(nonres_sb, os.path.join(OUTPUT_DIR, "non-residential-singbox.json"))
    ...
    print(f"[*] 导出完毕: 全量 {len(all_links)} | 家宽 {len(res_links)} | 非家宽 {len(nonres_links)}")
    return len(all_links), len(res_links), len(nonres_links)
...
def update_readme(total_count, res_count, nonres_count):
...
def main():
    ...
    report = build_run_report(previous_active, unique_nodes, test_results, len(tested_candidates),
                              len(raw_nodes), parsed_candidates_count, len(residential))
    write_run_report(report)
    print_run_report(report)
    update_readme(total, res, nonres)
